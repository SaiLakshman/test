...
// peer chaincode invoke -o <ORDERER_ENDPOINT> -n headervoice -C chheader  -c '{"args":["rh","{\"hid\":\"QT041111111111111101\",\"peid\":\"A11111111101\",\"cname\":\"OLAPAY\",\"cli\":\"BLOCKCUBE\",\"ctgr\":\"8\",\"cts\":\"456789\",\"uts\":\"456787678\"}"]}'

// peer chaincode invoke -o <ORDERER_ENDPOINT> -n headervoice -C chheader  -c '{"args":["rbh","{\"hid\":\"QT041111111111111102\",\"peid\":\"A11111111101\",\"cname\":\"OLAPAY\",\"cli\":\"BLOCKCUBE2\",\"ctgr\":\"8\",\"cts\":\"456789\",\"uts\":\"456787678\"}","{\"hid\":\"QT041111111111111103\",\"peid\":\"A11111111102\",\"cname\":\"OLAPAY\",\"cli\":\"BLOCKCUBE3\",\"ctgr\":\"8\",\"cts\":\"456789\",\"uts\":\"456787678\"}"]}'
...
// peer chaincode invoke -o <ORDERER_ENDPOINT> -n headervoice -C chheader  -c '{"args":["bhe","55"]}'

// peer chaincode invoke -o <ORDERER_ENDPOINT> -n headervoice -C chheader  -c '{"args":["bbh","BLOCKCUBE1","BLOCKCUBE2"]}'
...
type Header struct {
    ...
    Status 			  string `json:"sts"`	// sts     : Either A/I
	...
	CommunicationMode string `json:"cmode"` // cmode   :
}
...
var dltDomainNames = map [string]string {
	...
	"org2"		 : "Org2", // for testing in local
}
...
func (t *HeaderChainCode) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	switch function {
		...
		default:
			logger.Errorf("Received Unknown Function invocation : Available Function : rh , rbh , uhs, qh, hfh, qhwp, ra, bhe, bbh")
			return shim.Error("Received Unknown Function invocation : Available function : rh , rbh , uhs, qh, hfh, qhwp, ra, bhe, bbh")
		}
}
	...
	if len(data) == 5 {
		...
		if strings.Compare(existingUpdatedBy,creatr)==0{
			...
			if header.Status == "I" {
				...
				logger.Errorf("Header is still active with peid : " + header.PrincipleEntityId + "  Please set header status to Inactive before reassigning")
				return shim.Error("Header is still active with peid : " + header.PrincipleEntityId + " Please set header status to Inactive before reassigning")
			}
...
func (t *HeaderChainCode)  blacklistBulkHeaders(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	for i:=0; i<len(args); i++ {
		...
		resultData := map[string]interface{}{
			...
			"headerRejected": headerRejected,
			"headerBlacklisted":   headerDeleted, 
			"message" : "Bulk Header has been set blacklisted",
			"countSuccess":  strconv.Itoa(recordcount),
		}
