/*
This Chaincode is written for storing,retrieving,deleting the Content Templates that are stored in DLT
*/
...
//Logger for Logging
var logger = shim.NewLogger("CONTENT-TEMPLATES-CHAINCODE")
...
var categoryType = map[string]bool{
	"0": true,
	"1": true,
	"2": true,
	"3": true,
...
var dltDomainNames = map[string]string{
	...
	"tata.com":               "TA", //TATA
	"jio.com":                "JI", //JIO
	"org1":       "Org1", //for testing in local
	"org2":		  "Org2", //for tesing in local
}
var statusForAllDomain = map[string]string{
	"AI":   "A",
	"VO":   "A",
	"BL":   "A",
	"ML":   "A",
...
func (dlt *TemplateMgmtChaincode) setTemplate(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if _, flag := data["urn"].(string); !flag {
		...
		logger.Errorf("setTemplate:" + string(jsonResp))
	...
	if !validEnumEntry(data["ttyp"].(string), tempType) {
		jsonResp = "{\"Error\":\"Please enter one of these value for TemplateType 'CTSMS' or 'CTVOICE' or 'CSSMS' or 'CSVOICE' \"}"
		logger.Errorf("setTemplate:" + string(jsonResp))
		return shim.Error(jsonResp)
	}
	if data["ttyp"].(string) == "CTSMS" || data["ttyp"].(string) == "CTVOICE" {
		...
		if _, flag := data["vars"].(string); !flag {
			jsonResp = "{\"Error\":\"vars is empty\"}"
			logger.Errorf("setTemplate:" + string(jsonResp))
			return shim.Error(jsonResp)
		}

		if _, flag := data["coty"].(string); !flag {
			jsonResp = "{\"Error\":\"coty is empty\"}"
			logger.Errorf("setTemplate:" + string(jsonResp))
			return shim.Error(jsonResp)
		}
		...
		if !validEnumEntry(data["ctgr"].(string), categoryType) {
			jsonResp = "{\"Error\":\"Please enter any one from 0 to 8 for category\"}"
			logger.Errorf("setTemplate:" + string(jsonResp))
			return shim.Error(jsonResp)
		}

	}
	if data["ttyp"].(string) == "CSSMS" || data["ttyp"].(string) == "CSVOICE" {
	...
	if value != nil {
		...
		TemplateStruct.ObjType = "ContentTemplates"
		...
		TemplateStruct.Status = statusForAllDomain
...
func (dlt *TemplateMgmtChaincode) addBatchTemplates(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; i < len(args); i++ {
		...
		if err != nil {
			...
			failed_urnerr = append(failed_urnerr, "Input argument unmarshaling error")
			continue
		}
		//madatory field validation
		if _, flag := data["urn"].(string); !flag {
			...
			failed_urnerr = append(failed_urnerr, "urn is empty")
			continue
		}
		if _, flag := data["peid"].(string); !flag {
			jsonResp = "{\"Error\":\"peid is empty\"}"
			...
			failed_urnerr = append(failed_urnerr, "peid is empty")
			continue
		}
		if _, flag := data["tname"].(string); !flag {
			jsonResp = "{\"Error\":\"tname is empty\"}"
		...
		if _, flag := data["ttyp"].(string); !flag {
			...
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "ttyp is empty")
			continue
		}
		if _, flag := data["tcont"].(string); !flag {
			jsonResp = "{\"Error\":\"tcont is empty\"}"
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "tcont is empty")
			continue
		}
		if _, flag := data["cts"].(string); !flag {
			jsonResp = "{\"Error\":\"cts is empty\"}"
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "cts is empty")
			continue
		}
		if _, flag := data["uts"].(string); !flag {
			jsonResp = "{\"Error\":\"uts is empty\"}"
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "uts is empty")
			continue
		}
		if _, flag := data["cli"].([]interface{}); !flag {
			jsonResp = "{\"Error\":\"cli is empty\"}"
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "cli is empty")
			continue
		}
		cliList := data["cli"].([]interface{})
		cli := make([]string, len(cliList))
		...
		if _, flag := data["ctyp"].(string); !flag {
			...
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "ctyp is empty")
			continue
		}
		if !validEnumEntry(data["ttyp"].(string), tempType) {
			jsonResp = "{\"Error\":\"Please enter one of these value for TemplateType 'CTSMS' or 'CTVOICE' or 'CSSMS' or 'CSVOICE' \"}"
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "Please enter one of these value for TemplateType 'CTSMS' or 'CTVOICE' or 'CSSMS' or 'CSVOICE'")
			continue
		}
		...
		if data["ttyp"].(string) == "CTSMS" || data["ttyp"].(string) == "CTVOICE" {
			if _, flag := data["ctgr"].(string); !flag {
				...
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "ctgr is empty")
				continue
			}
			if _, flag := data["vars"].(string); !flag {
				jsonResp = "{\"Error\":\"vars is empty\"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "vars is empty")
				continue
			}
			if _, flag := data["coty"].(string); !flag {
				jsonResp = "{\"Error\":\"coty is empty\"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "coty is empty")
				continue
			}
			if len(data["ctgr"].(string)) == 0 {
				jsonResp = "{\"Error\":\" Input Data: Category is empty; for Template Type 'Content Template' category is mandatory\"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Category is empty")
				continue
			}
			if !validEnumEntry(data["ctyp"].(string), communicationTypeForCT) {
				jsonResp = "{\"Error\":\"Please enter one of these value for communicationType 'p','T','SE' or 'SI' \"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Please enter one of these value for communicationType 'p','T','SE' or 'SI'")
				continue
			}
			if !validEnumEntry(data["ctgr"].(string), categoryType) {
				jsonResp = "{\"Error\":\"Please enter any one from 0 to 8 for category  \"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Please enter any one from 0 to 8 for category")
				continue
			}
		}
		if data["ttyp"].(string) == "CSSMS" || data["ttyp"].(string) == "CSVOICE" {
			if _, flag := data["csty"].(string); !flag {
				...
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "csty is empty")
				continue
			}
			if !validEnumEntry(data["csty"].(string), consentTemplateType) {
				jsonResp = "{\"Error\":\"Please enter one of these value for consent template type '1' or '2' or '3' \"}"
				logger.Errorf("batchTemplates:" + string(jsonResp))
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Please enter one of these value for consent template type '1' or '2' or '3' ")
				continue
			}
			if _, flag := data["ctyp"].(string); !flag {
				jsonResp = "{\"Error\":\"ctyp is empty\"}"
		...
		if _, err := strconv.Atoi(data["urn"].(string)); err != nil {
			...
			logger.Errorf("batchTemplates:" + string(jsonResp))
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "URN is not numeric")
			continue
		}
		...
		if err != nil {
			...
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "GetState Failed for Template Error : "+string(err.Error()))
			continue
		}
		...
		if value != nil {
			...
			failed_urn = append(failed_urn, data["urn"].(string))
			failed_urnerr = append(failed_urnerr, "Template already exists. Please choose unique TemplateID")
			continue
		} else {
			TemplateStruct := &Template{}
			...
			if err != nil {
				...
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Marshalling Error : "+string(err.Error()))
				continue
			}
			//Inserting DataBlock to BlockChain
			err = stub.PutState(TemplateStruct.TemplateID, TemplateAsBytes)
			if err != nil {
				...
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "PutState Failed Error : "+string(err.Error()))
				continue
			}
			...
			if err != nil {
				...
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Event Payload Marshalling Error :"+string(err.Error()))
				continue
			}
			...
			if err2 != nil {
				...
				failed_urn = append(failed_urn, data["urn"].(string))
				failed_urnerr = append(failed_urnerr, "Event Creation Error for EventID : "+string(EVTADDTEMPLATE))
				continue
			}
	...
	resultData := map[string]interface{}{
		...
		"failed_urnerr": failed_urnerr,
		"message":       "Add Batch Template Success",
	}
	respJson, _ := json.Marshal(resultData)
	return shim.Success(respJson)
}
...
func (dlt *TemplateMgmtChaincode) updateTemplateStatus(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if value == nil {
		...
		existingStatus = Template.Status

		switch args[1] {
		case "A":
			if existingStatus[dltNode] == "I" {
				existingStatus[dltNode] = "A"
			} else {
				logger.Errorf("Template is already Active")
				return shim.Error("Template is already Active")
			}
		case "I":
			if existingStatus[dltNode] == "A" {
				existingStatus[dltNode] = "I"
			} else {
				logger.Errorf("Template is already Inactive")
				return shim.Error("Template is already Inactive")
			}
		default:
		...
		Template.Status = existingStatus
		...
		if err != nil {
			logger.Errorf("updateTemplateStatus : Marshalling Error : " + string(err.Error()))
			return shim.Error("updateTemplateStatus : Marshalling Error : " + string(err.Error()))
		}
		//Inserting DataBlock to BlockChain
