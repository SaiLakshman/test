...
func isValidPurpose(purpose string) (bool, string) {
	if !validEnumEntry(purpose, purposeValues) {
		return false, "Purpose can be either 1(Promotional) or 2(Service)"
	}
	return true, ""
}
	...
	for _, eachConsent := range consents {
		...
		if len(consents) > 0 {
			...
			for _, cons := range consents {
				if cons.Status == _ConsentRaisedStatus {
					flag = true
				}
			}
			if flag {
				_consentLogger.Infof(_Format4)
				e := ErrorData{ID: eachConsent.ConsentID, Msg: _Format4 + "Status is 1."}
				fConsents = append(fConsents, e)
				continue
			}

			if !flag {
				_consentLogger.Infof("Warning:Consent(s) with status either 2 / 3 / 4 is already there in Ledger")
			}
		}
		//validation
...
//"trxnID"		:  transaction Id
//"consentID"	:  Consent Id for which data has been modified
//"status"		:  "Update Consent Purpose Successful"
//"consent"		:  Updated consent element
