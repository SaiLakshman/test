...
//creating Header record in the ledger
...
//queryByHeaderId fetches the record by the headerId from the ledger.
func (s *HeaderVoiceManager) queryByHeaderId(stub shim.ChaincodeStubInterface) peer.Response {
	_, args := stub.GetFunctionAndParameters()
	if len(args) < 1 {
...
//update status function updates the status of the header given headerId
...
func (s *HeaderVoiceManager) getHistoryByHeaderId(stub shim.ChaincodeStubInterface) peer.Response {
	_, args := stub.GetFunctionAndParameters()
	if len(args) < 1 {
		return shim.Error("Invalid arguments provided")
	}
...
func (s *HeaderVoiceManager) getByAnyKeyWithPagination(stub shim.ChaincodeStubInterface) peer.Response {
	type Query struct {
