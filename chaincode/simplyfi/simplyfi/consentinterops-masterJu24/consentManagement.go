...
//TotalBulkResponse holds Success, Error consents and Phone details
type TotalBulkResponse struct {
	SuccesConsents []SuccessData `json:"successData"`
	FailedConsents []ErrorData   `json:"failedData"`
	SuccessPhones  []string      `json:"sucessPhone"`
}
...
func isValidConsent(c Consentdetails) (bool, string) {
	...
	isOK, msg := isValidMsisdn(c.Msisdn)
	if !isOK {
		return false, msg
	}
...
func isValidMsisdn(Msisdn string) (bool, string) {
	...
	if !regex.MatchString(Msisdn) {
		return false, "Invalid MSISDN. MSISDN needs to a numeric number between 10 to 13 without Country Code."
	}
	return true, ""
}
...
//RecordConsentInBulk records the Consents in Bulk. It will make status of all the consents as "Approved (2)", default Purpose as "1".
//Returned payload contains three blocks - 'failedData', 'successData' and 'sucessPhone'. FailedData contains an array of map with Falure details, SuccessData contains an array of map with details of Successfully Saved Consent and SucessPhone will contains the array of successful phones saved in ledger. This functionality has less validation.
//--FailedData block is as below:
//"errormsg": <Reason for Failure>,
//"id"		: <URN>
//--SuccessData block is as below:
...
//"message": "Consent Creation Successful",
//"trxnId": <transactionid>
//--SucessPhone block is as follow:
//array of success phone number
func (cm *ConsentManager) RecordConsentInBulk(stub shim.ChaincodeStubInterface) pb.Response {
	_consentLogger.Infof("Inside of RecordConsentInBulk.")
	_, args := stub.GetFunctionAndParameters()
	...
	for _, eachConsent := range consents {
		...
		/* consents := cm.getConsentsByMsisdnCliStatus(stub, eachConsent.Msisdn, eachConsent.Cli, "2")
		...
		if len(consents) > 0 {
			...
			if flag {
				...
				e := ErrorData{ID: "MSISDN:" + eachConsent.Msisdn + ",CLI:" + eachConsent.Cli, Msg: _Format4 + "Status is 2."}
				fConsents = append(fConsents, e)
				continue
			}
		} */
		...
		eachConsent.Creator = creater
		eachConsent.UpdatedBy = creater
		eachConsent.Purpose = _PurposeBoth
	...
	_consentLogger.Infof("Inside of RecordConsentInBulk.Time Taken", t2.Sub(t1))

	totalResponse := TotalBulkResponse{SuccesConsents: sConsents, FailedConsents: fConsents, SuccessPhones: phoneNos}
...
func (cm *ConsentManager) QueryConsentsWithPagination(stub shim.ChaincodeStubInterface) pb.Response {
	_consentLogger.Errorf("Within QueryConsentsWithPagination")
	_, args := stub.GetFunctionAndParameters()
	...
	if err != nil {
		_consentLogger.Errorf("Error while parsing the page size :" + string(err.Error()))
		jsonResp = "{\"error\":\"PageSize parsing error- \"" + string(err.Error()) + "\"}"
		return shim.Error(jsonResp)
	}
	bookmark := args[2]
	resultsIterator, responseMetaData, err := stub.GetQueryResultWithPagination(queryString, int32(pageSize), bookmark)
	if err != nil {
		_consentLogger.Errorf("GetQueryResultWithPagination Failed :" + string(err.Error()))
		jsonResp = "{\"error\":\"GetQueryResultWithPagination Failed- \"" + string(err.Error()) + "\"}"
		return shim.Error(jsonResp)
	}
	...
	for resultsIterator.HasNext() {
		...
		if err != nil {
			_consentLogger.Errorf("Unable to unmarshal Consents retrieved :" + string(err.Error()))
			jsonResp = "{\"error\":\"Unable to unmarshal Consents retrieved- \"" + string(err.Error()) + "\"}"
			return shim.Error(jsonResp)
		}
